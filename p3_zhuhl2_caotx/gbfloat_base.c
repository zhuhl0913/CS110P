...
void normalize_FVec(FVec v) // initialize sum array
{
    // float sum = 0.0;
    unsigned int i,j;
    int ext = v.length / 2;
...
float* get_pixel(Image img, int x, int y)   // return the address of the initial position of the (x,y) element
{
    if (x < 0)
    {
        x = 0;
...
float gd(float a, float b, float x) // a for variance, b for expectation -> this is a one-dimensional gaussian function
{
    float c = (x-b) / a;
...
FVec make_gv(float a, float x0, float x1, unsigned int length, unsigned int min_length)
{
    ...
    if(v.min_length > v.length){
        v.min_deta = 0;
    }else{  // v.min_length <= v.length
        v.min_deta = ((v.length - v.min_length) / 2);
    }
    v.data = malloc(length * sizeof(float));
    v.sum = malloc((length / 2 + 1)* sizeof(float));    // ext + 1
...
Image img_sc(Image a)   // image copy
{
    Image b = a;
