...
int main(int argc, char** argv)
{
    ...
    double delta_p = 0.0; double delta_n = 0.0;
    for (int y = 0; y < Desimg.dimX* Desimg.dimY * Desimg.numChannels; y++)
    {
        ...
        delta_p += (fabs(Desimg.data[y] - Srcimg.data[y]) > 1) && (Desimg.data[y] - Srcimg.data[y]) > 0 ? (Desimg.data[y] - Srcimg.data[y]) : 0;
        delta_n += (fabs(Desimg.data[y] - Srcimg.data[y]) > 1) && (Desimg.data[y] - Srcimg.data[y]) < 0 ? (Desimg.data[y] - Srcimg.data[y]) : 0;
    }
    avg_delta = delta / (Desimg.dimX* Desimg.dimY * Desimg.numChannels);
    double avg_delta1 = delta_p / (Desimg.dimX* Desimg.dimY * Desimg.numChannels);
    double avg_delta2 = delta_n / (Desimg.dimX* Desimg.dimY * Desimg.numChannels);
    printf("%lf %lf %lf/n", avg_delta,avg_delta1,avg_delta2);
